...
<a
	class="flex flex-row justify-start items-center gap-1 color-alt-text btn py-0.5 rounded-lg px-0.5 mr-2
...
>
	...
	{:else if channel.type === ChannelType.GuildText}
		{#if channel.id === guild.rules_channel_id}
			<svg
				...
				class="w-4 h-4"
				><path
					fill="oklab(0.712887 -0.00515984 -0.0109761)"
					fill-rule="evenodd"
	...
	<span class:text-xs={channel.type === ChannelType.GuildCategory}
		>{channel.name.slice(0, 20)}{channel.name.length > 20 ? '...' : ''}</span
	>
</a>

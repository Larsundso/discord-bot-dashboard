...
>
	...
	<form class="flex flex-col gap-2 justify-center items-center gap-5" action="?/login" method="POST">
		<div class="relative">
			<input
				type="password"
				placeholder="Token"
			...
			{#if form?.incorrect}<p class="color-danger absolute text-sm top-5 left-1">Invalid token!</p>{/if}
		</div>
